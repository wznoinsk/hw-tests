...
# /etc/rc.local will not be created using 'personality' features if nova.conf doesn't have inject_partition set
# correctly (to allow personality) also, even with above enabled libguestfs has issues with cirros images
# better not rely on "personality" enabled in openstack
# 'if [ -f /etc/rc.local ]; then echo "/ete/rc.local exist"; else echo "/ete/rc.local not exist"; fi',
# 'while [ ! -f /etc/rc.local ]; do echo "waiting for /etc/rc.local"; sleep 0.1; done;',
# 'chmod a+x /etc/rc.local',i
...
CONSOLE_DATA = [
    ...
    'echo -e "'
        ...
        'lspci -n > /dev/console\n'
        'echo \\\"%s\\\" > /dev/console" | '
        'sudo tee /etc/rc.local' % ((PCIINFO_DELIMITER % "RC LSPCI BEGIN"), (PCIINFO_DELIMITER % "RC LSPCI END")),
